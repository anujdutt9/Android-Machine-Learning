...
freeze_graph.freeze_graph(input_graph='./saved_model/weather_prediction.pbtxt',
                          input_saver='',
                          input_binary=True,
                          input_checkpoint='./saved_model/weather_prediction.ckpt',
                          output_node_names='y_out',
                          restore_op_name='save/restore_all',
                          filename_tensor_name='save/Const:0',
                          output_graph='./saved_model/frozen_weather_prediction.pb',
                          clear_devices=True,
                          initializer_nodes='',
...
with tf.gfile.Open('./saved_model/frozen_weather_prediction.pb', 'rb') as f:
...
output_graph_def = optimize_for_inference_lib.optimize_for_inference(input_graph_def=input_graph_def,
                                                                     input_node_names=["input_features"],
                                                                     output_node_names=['y_out'],
...
converter = tf.lite.TFLiteConverter.from_frozen_graph(graph_def_file="./saved_model/frozen_weather_prediction.pb",
                                                      input_arrays=["input_features"],
                                                      output_arrays=['y_out'],
                                                      input_shapes={'input_features': [1, 4]})
